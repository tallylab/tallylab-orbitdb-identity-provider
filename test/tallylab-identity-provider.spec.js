...
describe('Identity Provider', function () {
  ...
  it('generates deterministic keys based on 32-byte seed strings', () => {
    ...
    const keypair1 = identities.TallyLabIdentityProvider.keygen(nacl, seed1)
    ...
    const keypair2 = identities.TallyLabIdentityProvider.keygen(nacl, seed2)
    ...
    const keypair3 = identities.TallyLabIdentityProvider.keygen(nacl, seed3)
    ...
    const keypair4 = identities.TallyLabIdentityProvider.keygen(nacl, seed4)
  ...
  it('returns security version 1.1 if a random keygen is used', () => {
    const randomKeypair = identities.TallyLabIdentityProvider.keygen(nacl)
  ...
  it('can be added to Identities as a valid identity provider', () => {
    assert(identities.Identities.isSupported(identities.TallyLabIdentityProvider.type))
  })
  ...
  it('creates a valid identity object', async () => {
    ...
    assert(identity.provider instanceof identities.Identities)
  ...
  it('verifies a proper identity object', async () => {
    ...
    assert(await identities.TallyLabIdentityProvider.verifyIdentity(identity))
    await identity.provider._keystore.close()
  })
  ...
  it('exports identity in JSON.stringified format', async () => {
    ...
    const keypair = identities.TallyLabIdentityProvider.keygen(nacl, seed)
